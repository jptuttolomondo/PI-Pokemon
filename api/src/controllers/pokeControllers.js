...
const getPokemonsByName=routesPoke.get( `/pokemon/`,async (req,res,next)=>{
...
else{ for(let i=0;i<types.length;i++){
const typePokemon= await Type.findAll(
  {where:
  {name:types[i]}
  }
)
  console.log('typePokemon',typePokemon)//ver como se haria con mas de un tipo econ un array
await newPoke.addType(typePokemon)
          }
